# One-sentence paper reviews



These notes are generated from the curated ANC-only bibliography and OpenAlex abstracts where available.



1. (Nelson et al., 1990) ANC foundation/application. Previous work on the active control of sound has mostly used frequency domain formulations in order to establish the physical limitations of active methods

2. (Eriksson, 1990) FxLMS/adaptive-filter foundation. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

3. (Tichý, 1991) ANC foundation/application. Active control of sound fields is an emerging technology which has a great potential to become superior to many passive low frequency noise control methods

4. (Boucher et al., 1991) ANC foundation/application. The paper presents an investigation into the effects of errors in the assumed plant model on the performance of both single and multichannel adaptive algorithms for the active control of sound and vibration
...
28. (Beemanpally et al., 2010) Multichannel/distributed ANC. This paper presents a new active noise control system for infant incubators in neonatal intensive care units

29. (Zhang et al., 2010) Neural/fuzzy adaptive control. Most existing active noise control (ANC) methods usually require the precise model of secondary path, thus the performance may be deteriorated if the identified secondary path is inaccurate

30. (Duan, 2011) ANC foundation/application. Noise, vibration, and harshness (NVH) has been an important factor in the development of modern motor vehicles since the 1980s
...
54. (Ghasemi et al., 2015) Nonlinear adaptive filtering. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

55. (Guo et al., 2016) Nonlinear adaptive filtering. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.
...
60. (Le et al., 2017) Neural/fuzzy adaptive control. Models nonlinear ANC paths using structured nonlinear adaptive filters.

61. (Zhao et al., 2017) FxLMS/adaptive-filter foundation. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

62. (Ferrer et al., 2017) Multichannel/distributed ANC. In this paper, we present a distributed affine projection (AP) algorithm for an acoustic sensor network where the nodes are acoustically coupled

63. (Srazhidinov et al., 2017) Nonlinear adaptive filtering. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

64. (Murata et al., 2017) FxLMS/adaptive-filter foundation. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

65. (Le et al., 2018) Neural/fuzzy adaptive control. Uses functional-link neural expansions for nonlinear ANC with lower complexity than deep models.

66. (Guo et al., 2018) Nonlinear adaptive filtering. Models nonlinear ANC paths using structured nonlinear adaptive filters.

67. (Liu et al., 2018) Nonlinear adaptive filtering. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

68. (Guo et al., 2018) Nonlinear adaptive filtering. In many practical active noise control systems, the nonlinear secondary path (NSP) model can offer broader applications than the linear secondary path (LSP) model for the widespread existence of nonlinearities in the...

69. (Le et al., 2019) Neural/fuzzy adaptive control. Applies fuzzy/neuro-fuzzy learning to nonlinear or variable-step ANC adaptation.

70. (Guo et al., 2019) Nonlinear adaptive filtering. Models nonlinear ANC paths using structured nonlinear adaptive filters.

71. (Le et al., 2019) Neural/fuzzy adaptive control. Uses functional-link neural expansions for nonlinear ANC with lower complexity than deep models.

72. (Park et al., 2019) Neural/fuzzy adaptive control. Active noise control, or adaptive noise cancellation, techniques (ANC) attempt to reduce sound pressure level by generation and superposition of an anti-noise signal in order to improve human safety or comfort in noisy...

73. (Zhang et al., 2020) Deep/generative learning. The traditional adaptive filter and Filtered-x LMS (FxLMS) algorithm have been widely used in active noise control(ANC) systems

74. (Deb et al., 2020) Nonlinear adaptive filtering. Linear-in-the-parameters nonlinear controllers are widely used for active noise cancellation (ANC) in the presence of nonlinearities in the system

75. (Mondal, 2020) FxLMS/adaptive-filter foundation. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

76. (Kukde et al., 2020) Nonlinear adaptive filtering. Active control of noise for multi-channel applications is affected by the existence of nonlinear primary and secondary paths

77. (Zhu et al., 2020) Nonlinear adaptive filtering. As a robust nonlinear similarity measure, the maximum correntropy criterion (MCC) has been successfully applied to active noise control (ANC) for impulsive noise

78. (Yang et al., 2020) FxLMS/adaptive-filter foundation. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.

79. (Chen et al., 2021) Deep/generative learning. Reduces reliance on explicit secondary-path modeling through deep learning.

80. (Li et al., 2021) Neural/fuzzy adaptive control. Applies fuzzy/neuro-fuzzy learning to nonlinear or variable-step ANC adaptation.
...
86. (Luo et al., 2022) Deep/generative learning. Refines the filtered-reference adaptive-filter backbone used by most ANC systems.
...
92. (Xiao et al., 2023) Nonlinear adaptive filtering. In this brief, a Nyström adaptive filter-based nonlinear active noise control (NANC) system is established to improve the noise reduction ability of the kernel filtered-x least mean square (KFxLMS) and random Fourier...
...
99. (Singh et al., 2024) Neural/fuzzy adaptive control. Multi-channel active noise control systems utilize multiple control units to cancel unwanted noise across different channels simultaneously

100. (Kranthi et al., 2024) Nonlinear adaptive filtering. Noise mitigation proves to be a challenging task for active noise control in the existence of nonlinearities

101. (Luo et al., 2024) Deep/generative learning. Uses reinforcement learning for generative fixed-filter ANC selection/control.

102. (Zhu et al., 2025) Deep/generative learning. The air-conditioner noise in a rehabilitation room can seriously affect the mental state of patients
...
106. (Bai et al., 2025) Deep/generative learning. Introduces a causal WaveNet-Volterra architecture for nonlinear ANC.

107. (Islam et al., 2026) Multichannel/distributed ANC. This paper presents an Error Innovation-based Adaptive Estimation Kalman Filter (eIAEKF) tailored for multi-channel Active Noise Control (ANC) systems using MIMO filtered-x structures

108. (Aboutiman et al., 2026) Deep/generative learning. Traditional active noise control (ANC) systems rely on adaptive filtering techniques that assume a linear relationship between signals

109. (Yang et al., 2026) Nonlinear adaptive filtering. Relevant ANC contribution; inspect full paper for detailed methods and experimental conditions.

110. (Li et al., 2026) Deep/generative learning. Active Noise Control (ANC) deployment is often hindered by hardware nonlinearity and strict memory constraints
